...
module.exports.index = function(directory){
  ...
  process.emit('info', 'Catalog scanned');

	files = files.filter(file =>{
		let name = _.lowerCase(file);
		return _.endsWith(name, 'cbz') || _.endsWith(name, 'cbr') || _.endsWith(name, 'zip');
	})
  ...
  process.emit('info', 'Scanning covers.')

	//Index covers
	let covers =  index_covers( files );
	return covers.then( ()=> {
		console.log('Indexing complete...');
    process.emit('info', 'Indexing complete...')
		return {folders, archives}
	}, err =>{
		console.error(`Error indexing covers ${err.message}`);
    process.emit('info', `Error indexing covers ${err.message}`)
 ...
 function index_cover(file){
	...
	let key = path.basename(file);
 	return page(file, COVER_PAGE, THUMBNAIL_SIZE).then(data =>{
 		...
 		console.error(`Error storing cover for ${key}, ${err.message}`);
 		debugger;
 		page(file, COVER_PAGE, THUMBNAIL_SIZE);
