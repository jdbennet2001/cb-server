...
async function index(directory){
  ...
  console.log( `Scan complete.., ${_.size(files)} files found`)
  ...
  folders = _.map(folders, folder =>{
    ...
    let parent = path.resolve(folder, '..');
...
function index_file(file){
  ...
  let fileSizeInBytes = size(file);
  let publishYear = year(name);
	...
	try{
		...
		return {length, name ,location, directory, fileSizeInBytes, publishYear};
	}catch(err){
...
/*
 Return the year encoded in a file name
 */
 function year(file){

  let years = _.range(1930, 2020);

  let publish_year = years.find(year =>{
    return _.includes(file, JSON.stringify(year));
  });
