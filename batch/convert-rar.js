...
const {files} = scan_directory('/Volumes/VIDEO/comics');

const archives = files.filter(file =>{
    return _.endsWith(file, 'cbr');
})

console.log(`${_.size(archives)} found`);

let conversion = _.reduce(archives, function(promise_chain, current_task, index) {
    return promise_chain.then( () =>{
        return convert(current_task, tmp_dir);
    });
}, Promise.resolve() );

conversion.then( () =>{
    console.log(`Complete, ${_.size(archives)} converted.`)
}, err =>{
