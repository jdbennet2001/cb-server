...
let queries = _.reduce(weeks, function(promise_chain, current_week, index) {
    return promise_chain.then( () =>{
        console.log( `Checking ${current_week}, ${index} of ${_.size(weeks)}`);
...
function get_data(issue_number){
    ...
    let cache_path = path.join(__dirname, `../data/catalog-db/${_.padStart(issue_number, 3, '0')}.json`);
